...
---
# Redis持久化机制
Redis的主要是基于Key- Value结构设计存储系统，需要部署在服务器上或者对应的计算机上，其数据全部都在内存之中。一般来说，系统出现服务挂掉或者掉电，就会导致数据出现丢失的情况。Redis为了保障这种因为故障而丢失数据的情况，提供了一种“持久化“的机制，即就是我们常说的Redis‍持久化实现(简洁的说，就是把内存中的数据存入到磁盘)。‍总的来说，Redis持久化实现方案主要有AOF和RDB两种实现方式，其中：
