...
---
# JVM基本结构
Java虚拟机那么负责？它的基本结构是什么？各个组成部分是什么？各部分是如何协调工作的。让我们带着问题去一起学习吧。
...
在Java栈中保存的主要内容为栈帧，每一次函数调用，都会有一个对应的栈帧被压入Java栈，每一个函数调用结束，都会有一个栈帧被弹出Java栈。在一个栈帧中，至少包含局部变量表、操作数栈和帧数据区几个部分。
