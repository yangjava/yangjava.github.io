...
例如：我们创建一个Spring项目，项目主要用来做登录。应用名称叫做login。那么在apollo中就是Application(应用)的名称时login。写作appId=login。

我们项目分为测试环境（test）和生产环境（online）那么就是两套环境。环境名称分别时测试环境test。生产环境online。

在测试环境下按照不同的数据中心分为北京集群和上海集群。那么就是两个Cluster。

在集群下可以配置不同类别的文件。例如数据库配置文件DB。RPC配置文件Dubbo。等


Apollo在创建项目的时候，都会默认创建一个"application"的Namespace，"application"是个应用自身使用的。例如Spring Boot中项目的默认配置文件application.yaml，这里application.yaml就等同于"application"的Namespace。

对于大多数应用来说,"application"Namespace已经能满足日常配置使用场景客户端获取"application"Namespace的代码如下
```
...
tips: 非properties格式的namespace，在客户端使用时需要调用
```
...
- 关联类型(继承类型) 具有private权限，关联类型的Namespace继承于公共类型的Namespace，用于覆盖公共Namespace的某些配置。
